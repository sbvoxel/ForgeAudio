...
FORGE_INTERNAL_API void forge_linked_list_add_entry(ForgeLinkedList **start, void *toAdd, ForgeAudioMutex lock,
                                                    ForgeMallocFunc malloc_func);
FORGE_INTERNAL_API void forge_linked_list_prepend_entry(ForgeLinkedList **start, void *toAdd, ForgeAudioMutex lock,
                                                        ForgeMallocFunc malloc_func);
FORGE_INTERNAL_API void forge_linked_list_remove_entry(ForgeLinkedList **start, void *toRemove, ForgeAudioMutex lock,
...
typedef void(FORGE_AUDIO_CALL *ForgeAudioDecodeCallback)(ForgeVoice *voice, const void *src, float *decodeCache,
                                                         uint32_t samples);

typedef void(FORGE_AUDIO_CALL *ForgeAudioResampleCallback)(float *restrict dCache, float *restrict resampleCache,
                                                           uint64_t *resampleOffset, uint64_t resampleStep,
                                                           uint64_t toResample, uint8_t channels);

typedef void(FORGE_AUDIO_CALL *ForgeAudioMixCallback)(uint32_t toMix, uint32_t srcChans, uint32_t dstChans,
                                                      float *restrict srcData, float *restrict dstData,
...
/* Internal Functions */
FORGE_INTERNAL_API void forge_audio_insert_submix_sorted(ForgeLinkedList **start, ForgeSubmixVoice *toAdd,
...
FORGE_INTERNAL_API extern void forge_audio_resample_generic(float *restrict dCache, float *restrict resampleCache,
                                                            uint64_t *resampleOffset, uint64_t resampleStep,
                                                            uint64_t toResample, uint8_t channels);

FORGE_INTERNAL_API extern void (*forge_audio_amplify)(float *output, uint32_t totalSamples, float volume);
...
#define MIX_FUNC(name)                                                                                                 \
    FORGE_INTERNAL_API extern void forge_audio_mix_##name##_scalar(                                                    \
        uint32_t toMix, uint32_t srcChans, uint32_t dstChans, float *restrict srcData, float *restrict dstData,        \
        float *restrict coefficients);
MIX_FUNC(generic)
MIX_FUNC(1in_1out)
MIX_FUNC(1in_2out)
...
#define DECODE_FUNC(name)                                                                                              \
    FORGE_INTERNAL_API extern void forge_audio_decode_##name(ForgeVoice *voice, const void *src, float *decodeCache,   \
                                                             uint32_t samples);
DECODE_FUNC(pcm8)
DECODE_FUNC(pcm16)
DECODE_FUNC(pcm24)
...
FORGE_INTERNAL_API void forge_platform_init(ForgeAudioEngine *audio, uint32_t flags, uint32_t deviceIndex,
                                            ForgeAudioFormatExtensible *mixFormat, uint32_t *updateSize,
                                            void **platformDevice);
...
FORGE_INTERNAL_API ForgeResult forge_platform_get_device_details(uint32_t index, ForgeDeviceDetails *details);

/* Threading */

FORGE_INTERNAL_API ForgeAudioThread forge_platform_create_thread(ForgeAudioThreadFunc func, const char *name,
                                                                 void *data);
